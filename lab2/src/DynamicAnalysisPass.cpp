...
namespace instrument
{
  ...
  void instrumentBinOpOperands(Module *M, BinaryOperator *BinOp, int Line, int Col)
  {
     ...
     * operands.
     */
    char binop_symbol = getBinOpSymbol(BinOp->getOpcode());
    auto LineVal = ConstantInt::get(Int32Type, Line);
    auto ColVal = ConstantInt::get(Int32Type, Col);
    auto OpCharVal = ConstantInt::get(CharType, binop_symbol);

    std::vector<Value *> Args = {OpCharVal, LineVal, ColVal, BinOp->getOperand(0), BinOp->getOperand(1)};
